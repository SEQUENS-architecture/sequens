...
export default async function handler(req, res) {
  ...
  try {
    ...
    const coreSys = BASE + ` For ONE named step of a lesson, return a SMALL JSON object: {"teach":["a short paragraph of what the TEACHER does, for the plan"],"say":["an optional say-aloud line"],"slide":{"heading":"the ONE short line shown to PUPILS on the board for this step: a question or instruction in pupil-facing words, under 10 words, never the teacher instructions","prompt":"optional one short pupil-facing sub-line","task":["independent work only: short pupil task lines, else empty"],"stems":["optional sentence starters for pupils, else empty"],"options":["Quick check step only: 2 or 3 vote options like 'A ...', else empty"]},"modelling_space":{"purpose":"","detail":[""],"examples":{"label":"What usually comes up","items":["",""]},"warn":""},"misconception":null,"hinge":null}. The slide is PUPIL-FACING and minimal, a board a class reads, not the teacher prose. Only the 'Quick check' step has "hinge":{"misconception_caught":"","why_diagnostic":""} and fills slide.options with the vote choices. A step has "misconception":{"title":"","body":""} only where one truly bites, else null. For 'First thing' in maths, put a Flashback 4 in teach and a short do-now instruction in slide.heading.`;
